...
function getBooksFromGoogle(searchKey = 'coding') {
    ...
    return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchKey}`)
        .then(res => {
            ...
            const books = formatBooks(res.data)
            utilService.saveToStorage(GOOGLE_BOOKS_KEY, books)
            return books
        })
        // } else return Promise.resolve(utilService.loadFromStorage(GOOGLE_BOOKS_KEY));
}

function formatBooks(bigBooks) {
    console.log(bigBooks);
    return bigBooks.items.map(bigBook => {
        return {
            id: bigBook.id,
            title: bigBook.volumeInfo.title.slice(0, 15),
            listPrice: {
                amount: utilService.getRandomInt(),
                isOnSale: Math.random() > 0.5,
                currencyCode: utilService.getCurrencyCode(),
            },
            publishedDate: bigBook.volumeInfo.publishedDate,
            thumbnail: bigBook.volumeInfo.imageLinks.thumbnail || "http://books.google.com/books/content?id=nBuA0hmspdMC&printsec=frontcover&img=1&zoom=5&edge=curl&source=gbs_api",
            subtitle: bigBook.volumeInfo.subtitle,
            authors: bigBook.volumeInfo.authors,
            description: bigBook.volumeInfo.description,
            pageCount: bigBook.volumeInfo.pageCount,
            categories: bigBook.volumeInfo.categories,
            language: bigBook.volumeInfo.language
        }
    })
}
...
function doPaging(delta, bookId) {
    const bookIdx = gBooks.findIndex(book => book.id === bookId);
    if (delta === 1 && bookIdx === gBooks.length - 1) return Promise.resolve(gBooks[1])
    else if (delta === -1 && bookIdx === 0) return Promise.resolve(gBooks[gBooks.length - 1]);
    const book = gBooks[bookIdx + delta];
